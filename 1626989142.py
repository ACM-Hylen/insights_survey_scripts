...
class MigrationScript(BaseMigration):
    """
        Readiness Survey - 2021-2022
    """
    sql = None
    sql_dictionary = None

    SURVEY_TEMPLATE_INTERNAL_KEY = 'readiness_2021_2022'
    ...
    # Questions - There is an additional space after the word degree in all questions to ensure each is unique & uniform
    SURVEY_QUESTION_ONE = 'To what degree  do you intend to attend this institution for the upcoming academic term?'
    SURVEY_QUESTION_TWO = 'To what degree  do you feel you have been given the necessary information to start the term?'
    SURVEY_QUESTION_THREE = 'To what degree  do you have the technology resources to accommodate online classes? (Please skip if you do not have online classes.)'
    SURVEY_QUESTION_FOUR = "To what degree  are you confident that you can pay for this term's tuition and fees?"
    SURVEY_QUESTION_FIVE = 'To what degree  are you academically prepared for the upcoming term?'
    SURVEY_QUESTION_SIX = 'To what degree  are you academically motivated for the upcoming term?'
    SURVEY_QUESTION_SEVEN = 'To what degree  do you belong at this institution?'
    ...
    QUESTION_FIVE_NEXT_STEPS = "Support students that don't feel prepared to advance their academic progress. The transition that incoming students are experiencing is unprecedented and they may need additional support."
    QUESTION_SIX_TALKING_POINT_WEAKNESS = 'Student lacks motivation for the start of classes. Motivation struggles can be the result of a wide range of other issues. Therefore, early outreach to better understand the source of motivation struggles is important.'
    QUESTION_SIX_TALKING_POINT_STRENGTH = 'Student reports being motivated to begin classes.'
    SURVEY_QUESTION_SIX_NEXT_STEPS = 'Help students find what motivates them to complete their course and their degree. Discussing their motivation may help explore root causes and provide opportunities to connect the student to resources.'
    QUESTION_SEVEN_TALKING_POINT_WEAKNESS = 'Student does not feel like they belong at this institution. Feeling or achieving a sense of belonging may be more challenging as the institution responds to the pandemic and the need to social distance.'
    QUESTION_SEVEN_TALKING_POINT_STRENGTH = 'Student feels like they belong at this institution.'
    QUESTION_SEVEN_NEXT_STEPS = 'Sense of belonging is a strong predictor of retention. Help students make connections to peers and organizations on campus and in the community, whether in-person or virtually.'
    ...
    def __init__(self):
        ...
        self.sql = {
            1: f"""
                ...
                ('Readiness Survey - 2021-2022', '{self.SURVEY_TEMPLATE_INTERNAL_KEY}', '{self.SURVEY_HOVER_TEXT}', 5, "{self.CONSENT_STATEMENT}", 2, 2);
            """,
            3: f"""
            ...
            6: f"""
                ...
                (
                    (SELECT id FROM nano_survey_configuration.issue WHERE issue_text = '{self.ISSUE_ACADEMIC_PREPAREDNESS}' AND issue_type = '{self.ISSUE_TYPE_WEAKNESS}' AND question_bank_id = (SELECT id FROM nano_survey_configuration.question_bank WHERE question_text = '{self.SURVEY_QUESTION_FIVE}')),
                ...
                (
                    (SELECT id FROM nano_survey_configuration.issue WHERE issue_text = '{self.ISSUE_ACADEMIC_PREPAREDNESS}' AND issue_type = '{self.ISSUE_TYPE_STRENGTH}' AND question_bank_id = (SELECT id FROM nano_survey_configuration.question_bank WHERE question_text = '{self.SURVEY_QUESTION_FIVE}')),
                ...
                (
                    (SELECT id FROM nano_survey_configuration.issue WHERE issue_text = '{self.ISSUE_ACADEMIC_MOTIVATION}' AND issue_type = '{self.ISSUE_TYPE_WEAKNESS}' AND question_bank_id = (SELECT id FROM nano_survey_configuration.question_bank WHERE question_text = '{self.SURVEY_QUESTION_SIX}')),
                ...
                (
                    (SELECT id FROM nano_survey_configuration.issue WHERE issue_text = '{self.ISSUE_ACADEMIC_MOTIVATION}' AND issue_type = '{self.ISSUE_TYPE_STRENGTH}' AND question_bank_id = (SELECT id FROM nano_survey_configuration.question_bank WHERE question_text = '{self.SURVEY_QUESTION_SIX}')),
